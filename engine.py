...
class TetrisEngine:
	def __init__(self):
		...
		self.extetromino_distribution = range(1,20)	# Programmable:
		...
		self.width = 10															# Essential and programmable
		self.height = 20														# Essential and programmable
		...
		self.update_duration = 100																	# Progrmmable, and its progression too
		self.window.after(self.update_duration, lambda: self.update_step())	# callback
		self.move_down_duration = 1000															# Progrmmable, and its progression too
	...
	def new_game(self):
		...
		self.text_area.insert(tk_END,self.render())
		self.cursor = self.default_cursor
